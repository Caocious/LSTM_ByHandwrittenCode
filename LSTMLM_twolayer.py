...
class TextLSTM(nn.Module):
    def __init__(self):
        ...
        self.W_i = nn.Linear(out_features=n_hidden,in_features=emb_size,bias=False)
        self.b_i = nn.Parameter(torch.ones([n_hidden]))
        self.W_h = nn.Linear(out_features=n_hidden,in_features=n_hidden,bias=False)
        self.b_h = nn.Parameter(torch.ones([n_hidden]))
        self.W_i_2 = nn.Linear(out_features=n_hidden,in_features=n_hidden,bias=False)
        self.b_i_2 = nn.Parameter(torch.ones([n_hidden]))
        self.W_h_2 = nn.Linear(out_features=n_hidden,in_features=n_hidden,bias=False)
        self.b_h_2 = nn.Parameter(torch.ones([n_hidden]))
    ...
    def forward(self,X):
        ...
        for x_t in X:
            #第一层
            #print(x_t.shape)
            i_t=torch.sigmoid(self.W_i(x_t)+self.b_i+self.W_h(h_t_1)+self.b_h)
            #print("i_t:",i_t.shape)
            f_t=torch.sigmoid(self.W_i(x_t)+self.b_i+self.W_h(h_t_1)+self.b_h)
            #print("f_t:",f_t.shape)
            g_t=self.relu(self.W_i(x_t)+self.b_i+self.W_h(h_t_1)+self.b_h)
            #print("g_t:",g_t.shape)
            o_t=torch.sigmoid(self.W_i(x_t)+self.b_i+self.W_h(h_t_1)+self.b_h)
            #print("o_t:",o_t.shape)
            #print("c_t_1:",c_t_1.shape)
            c_t=f_t*c_t_1+i_t*g_t
            h_t=o_t*self.relu(c_t)
            #print("h_t:",h_t.shape)
            h_t_1=h_t
            c_t_1=c_t

            #第二层
            i_t=torch.sigmoid(self.W_i_2(h_t)+self.b_i_2+self.W_h_2(h_t_1_layer2)+self.b_h_2)
            f_t=torch.sigmoid(self.W_i_2(h_t)+self.b_i_2+self.W_h_2(h_t_1_layer2)+self.b_h_2)
            g_t=self.relu(self.W_i_2(h_t)+self.b_i_2+self.W_h_2(h_t_1_layer2)+self.b_h_2)
            o_t=torch.sigmoid(self.W_i_2(h_t)+self.b_i_2+self.W_h_2(h_t_1_layer2)+self.b_h_2)
            c_t_layer2=f_t*c_t_1_layer2+i_t*g_t
            h_t_layer2=o_t*self.relu(c_t_layer2)
            h_t_1_layer2=h_t_layer2
            c_t_1_layer2=c_t_layer2


        model = self.W(h_t_layer2) + self.b # model : [batch_size, n_class]
        return model
