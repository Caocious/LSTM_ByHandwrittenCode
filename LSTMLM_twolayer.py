...
def make_dict(train_path):
    ...
    word2number_dict["<unk_word>"] = 1#没见过的单词
    number2word_dict[1] = "<unk_word>"
    word2number_dict["<sos>"] = 2#开始
    number2word_dict[2] = "<sos>"
    word2number_dict["<eos>"] = 3#结尾
...
class TextLSTM(nn.Module):
    def __init__(self):
        ...
        self.W_ii = nn.Linear(out_features=n_hidden,in_features=emb_size)
        self.W_hi = nn.Linear(out_features=n_hidden,in_features=n_hidden)
        self.W_if = nn.Linear(out_features=n_hidden,in_features=emb_size)
        self.W_hf = nn.Linear(out_features=n_hidden,in_features=n_hidden)
        self.W_ig = nn.Linear(out_features=n_hidden,in_features=emb_size)
        self.W_hg = nn.Linear(out_features=n_hidden,in_features=n_hidden)
        self.W_io = nn.Linear(out_features=n_hidden,in_features=emb_size)
        self.W_ho = nn.Linear(out_features=n_hidden,in_features=n_hidden)
        self.W_ii_2 = nn.Linear(out_features=n_hidden,in_features=n_hidden)
        self.W_hi_2 = nn.Linear(out_features=n_hidden,in_features=n_hidden)
        self.W_if_2 = nn.Linear(out_features=n_hidden,in_features=n_hidden)
        self.W_hf_2 = nn.Linear(out_features=n_hidden,in_features=n_hidden)
        self.W_ig_2 = nn.Linear(out_features=n_hidden,in_features=n_hidden)
        self.W_hg_2 = nn.Linear(out_features=n_hidden,in_features=n_hidden)
        self.W_io_2 = nn.Linear(out_features=n_hidden,in_features=n_hidden)
        self.W_ho_2 = nn.Linear(out_features=n_hidden,in_features=n_hidden)
        self.sigmoid = nn.Sigmoid()
        self.tanh = nn.Tanh()
        self.W = nn.Linear(n_hidden, n_class)

    def forward(self,X):
        X = self.C(X).to(device)
        hidden = torch.zeros((n_hidden,n_hidden)).to(device)
        #print("hidden:",hidden.shape)
        memo = torch.zeros((n_hidden,n_hidden)).to(device)
        ...
        for x_t in X:
            ...
            #第二层
            i_t_2=self.sigmoid(self.W_ii_2(h_t)+self.W_hi_2(h_t_1_layer2))
            f_t_2=self.sigmoid(self.W_if_2(h_t)+self.W_hf_2(h_t_1_layer2))
            g_t_2=self.tanh(self.W_ig_2(h_t)+self.W_hg_2(h_t_1_layer2))
            o_t_2=self.sigmoid(self.W_io_2(h_t)+self.W_ho_2(h_t_1_layer2))
            c_t_layer2=f_t_2*c_t_1_layer2+i_t_2*g_t_2
            h_t_layer2=o_t_2*self.tanh(c_t_layer2)
            h_t_1_layer2=h_t_layer2
            c_t_1_layer2=c_t_layer2


        model = self.W(h_t_layer2) # model : [batch_size, n_class]
        return model
...
def train_LSTMlm():
    ...
    for epoch in range(all_epoch):
        ...
        if (epoch+1) % save_checkpoint_epoch == 0:
            torch.save(model, f'models/LSTMlm_twolayer_model_epoch{epoch+1}.ckpt')
...
if __name__ == '__main__':
    ...
    print("\nTest the LSTMLM……………………")
    select_model_path = "models/LSTMlm_twolayer_model_epoch5.ckpt"
